...
`/arxiv-translator` - Automatically download, translate, and compile arXiv papers
`/marker` - High-fidelity PDF to Markdown conversion (with LaTeX formulas)
`/translate-pdf` - Full PDF translation preserving layout
`/notebooklm` - Query NotebookLM via browser automation

**Content Curation & Maintenance:**
`/note_summary` - Automatically fetch URLs/files, summarize, and categorize into `60_笔记`
...
`/ai-research-digest` - AI research papers and digest
`/fix-links` - Fix ghost wikilinks in the knowledge base
